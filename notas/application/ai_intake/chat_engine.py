...
from notas.application.ai_intake.brief_tool_updates import (
    apply_nutrition_brief_patch,
    apply_preference_draft_to_brief,
    apply_profile_draft_to_brief,
    apply_proposal_preferences_to_brief,
)
...
def _apply_llm_tool_result_data_to_brief(brief: NutritionBrief, data: dict) -> NutritionBrief:
    ...
    if isinstance(profile_draft, dict):
        updated = apply_profile_draft_to_brief(updated, profile_draft)

    preference_draft = data.get("preference_draft")
    if isinstance(preference_draft, dict):
        updated = apply_preference_draft_to_brief(updated, preference_draft)

    proposal_preferences = data.get("proposal_preferences")
    if isinstance(proposal_preferences, dict):
        updated = apply_proposal_preferences_to_brief(updated, proposal_preferences)
