  ...
  return (
    <ScrollView accessibilityRole="tablist" contentContainerStyle={styles.tabs} horizontal showsHorizontalScrollIndicator={false}>
      {tabs.map((tab) => {
        ...
        return (
          <Pressable accessibilityRole="tab" accessibilityState={{ selected }} key={tab.key} onPress={() => onChange(tab.key)} style={({ pressed }) => [styles.tab, selected && styles.tabSelected, pressed && styles.pressed]}>
...
const styles = StyleSheet.create({
  ...
  tabLabel: { color: tokens.color.textMuted, fontSize: tokens.type.caption, fontWeight: "500" },
