...
export default function CalendarizedMealDetailScreen() {
  ...
  const [savingPreparedFood, setSavingPreparedFood] = useState(false);
  ...
  async function togglePreparedFood(foodKey: string) {
    ...
    try {
      ...
      const updatedToday = await apiRequest<TodayData>(`/api/v1/days/${dayId}/meals/${encodeURIComponent(mealKey)}/check-ins`, { body: JSON.stringify(payload), method: "POST" });
      if (updatedToday.day_id === dayId) {
        setExecution(normalizeMealExecution(updatedToday.meal_execution).find((item) => item.meal_key === mealKey) ?? null);
      } else {
        ...
        <EntityDetailSection title="Tabla de comparación entre alimentos">
          <FoodPanels
            editing={{
              ...
              onReorder: async (items: FoodPanelItem[]) => runWithStatus(
                () => mutateFoods(`/api/v1/program/days/${dayId}/meals/${encodeURIComponent(mealKey)}/foods/order`, { body: JSON.stringify({ ordered_keys: items.map((item) => item.id) }), method: "PUT" }),
                { loadingLabel: "Actualizando comida", successLabel: "Comida actualizada" },
              ),
