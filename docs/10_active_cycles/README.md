...
- `ai_assistant_client_memory_profile_objects_cycle.md`: baseline CM00-CM13 completado y extensión de alineación LLM-native en su gate final. CM19-CM23 alinean runtime, contexto, prompts, replays y frontera determinística; CM24 implementa el harness real. La octava ejecución live pasó todas las invariantes automáticas. La revisión humana detectó una repetición generada por el fallback local posterior a tools, no por el prompt principal. Decisión 0117 limita ese fallback a confirmar estado (`state_ack_only.v1`) y agrega una invariante de ritmo; queda pendiente un rerun dirigido de `cambio_de_direccion` y su aprobación UX para cerrar el ciclo.
